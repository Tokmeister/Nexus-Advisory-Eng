...
import { buildConfidence, buildConsultantNote, buildExecutiveSummary, buildFocusTags, buildIssueStatement, buildRiskSummary, buildRisks, scoreRisk } from './logic/textBuilder';
...
export function buildAdvisoryOutput(input: AdvisoryInput): AdvisoryRecord {
  ...
  const recommendations = Array.from(new Set([...recs.quickWins, ...overlay.actionTemplates, ...overlay.recommendationBias].map(clean).filter(Boolean))).slice(0, 4);
  ...
  const priority = normalisedUrgency === 'Critical' || normalisedUrgency === 'High' ? 'High' : normalisedUrgency === 'Medium' ? 'Medium' : 'Low';
...
function buildPrintHtml(output: AdvisoryRecord): string {
  ...
  const subtitle = escapeHtml(truncateText(output.summary, 420));
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .report-cover {
      height: 297mm;
      background: linear-gradient(180deg, var(--brand-900) 0%, var(--brand-800) 36%, var(--paper) 36%, var(--paper) 100%);
      display: flex;
      flex-direction: column;
      overflow: hidden;
    }
    .cover-top {
      padding: 16mm 16mm 8mm 16mm;
      color: #eef6ff;
      text-align: center;
    }
    .cover-logo {
    ...
    .report-section-header { margin-bottom: 4mm; }
    ...
    .report-list li,
    .report-roadmap-card li { margin-bottom: 2.5mm; }
    .report-tags {
      display: flex;
      flex-wrap: wrap;
      gap: 3mm;
  ...
  <div class="report-root">
    <section class="report-page report-cover">
        ...
        <div class="cover-card">
          ...
          <div class="score-grid">
            ...
            <div class="score-card">
              ...
              <div class="score-value" style="font-size:14px; line-height:1.25;">${escapeHtml(output.category)}</div>
      ...
      <div class="cover-footer">
        <div>Generated by NEXUS Advisory Engine v6 (Executive Edition)</div>
        <div>${escapeHtml(output.reportId)}</div>
      </div>
    </section>
    ...
    <section class="report-page">
      <div class="report-content">
        ...
        <section class="report-section">
          ...
          <div class="report-roadmap" style="grid-template-columns: repeat(3, 1fr); gap: 5mm;">
            <div class="report-roadmap-card">
              <small>First 30 days</small>
              <h3>${escapeHtml(output.roadmap.day30Title)}</h3>
              ${renderList(output.roadmap.day30)}
            </div>
            <div class="report-roadmap-card">
              <small>Day 31 to 60</small>
              <h3>${escapeHtml(output.roadmap.day60Title)}</h3>
              ${renderList(output.roadmap.day60)}
            </div>
            <div class="report-roadmap-card">
              <small>Day 61 to 90</small>
              <h3>${escapeHtml(output.roadmap.day90Title)}</h3>
              ${renderList(output.roadmap.day90)}
            </div>
          </div>
        </section>
      </div>
