...
function appendContextTags(input: AdvisoryInput, tags: string[]): string[] {
  const text = [input.client, input.challenge, input.goal, input.constraints, input.businessFunction, input.triggerSource, input.notes]
    .map(clean)
    .join(' ')
    .toLowerCase();
...
function applyHealthcareSubtypeBoost(input: AdvisoryInput, rule: Rule, baseScore: number): number {
  ...
  const hasPrivatePractice = privatePracticeSignals.some((term) => text.includes(term));
  const hasHospital = hospitalSignals.some((term) => text.includes(term));
  const hasClinic = clinicSignals.some((term) => text.includes(term));
  const hasSupplier = supplierSignals.some((term) => text.includes(term));
  const hasPharmacy = pharmacySignals.some((term) => text.includes(term));
...
function selectBestRule(input: AdvisoryInput, rules: Rule[]): ClassificationResult | null {
  const scored = rules
    .map((rule) => {
      ...
      const score = applyHealthcareSubtypeBoost(input, rule, baseScore);
      return { rule, score };
    })
